...
const buildDayOptions = (selectedDay) =>
    Object.entries(dayNames)
        .map(([val, label]) => {
            const sel = String(val) === String(selectedDay) ? ' selected' : '';
...
const addScheduleRow = (day = 1, start = '', end = '') => {
    ...
    tr.querySelector('.ac-del-schedule').addEventListener('click', () => tr.remove());
...
const addHolidayRow = (date = '', description = '') => {
    ...
    tr.querySelector('.ac-del-holiday').addEventListener('click', () => tr.remove());
    tbody.appendChild(tr);
};
